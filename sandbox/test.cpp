...
struct MoverComponent {
    ...
    static MoverComponent random()
    {
        return {
            .offset = (f32)rand() / RAND_MAX * (f32)PI,
            .initialY = (f32)rand() / RAND_MAX * 3.f + 1.f,
        };
    }
...
struct GameState {
    ...
    pom::Store store;
    pom::SpriteRenderer* spriteRenderer = nullptr;
...
POM_CLIENT_EXPORT void clientBegin(GameState* gs)
{
    ...
    for (u32 x = 0; x < SIZE; x++) {
        for (u32 z = 0; z < SIZE; z++) {
            ...
            auto mover = gs->store.addComponent<MoverComponent>(e) = MoverComponent::random();
            gs->store.addComponent<pom::SpriteRendererComponent>(e) = {
                .transform = pom::maths::mat4::translate(
                                 pom::maths::vec3((f32)x - (f32)SIZE / 2, mover.initialY, (f32)z - (f32)SIZE / 2))
                             * pom::maths::mat4::scale(0.95f),
                .color = pom::Color::rgba(x * (255.f / 100.f), 0, z * (255.f / 100.f), 0.8),
            };
        }
    }

    gs->spriteRenderer = new pom::SpriteRenderer(pom::Application::get()->getMainWindow().getContext(), gs->store);
    ...
    POM_PROFILE_SCOPE("update");
    {
        if (!pom::Application::get()->getMainWindow().isMinimized()) {
            ...
            *config = gs->gridConfig;
            ...
            for (auto [_, sr, m] : gs->store.view<pom::SpriteRendererComponent, MoverComponent>()) {
                sr.transform[3][3] = m.initialY + tanf(m.offset + (f32)frame / 250.f);
            }

            gs->spriteRenderer->render(context, gs->commandBuffer, gs->camera.projection * gs->camera.view);
...
{
    delete gamestate->spriteRenderer;
    delete gamestate;
}
